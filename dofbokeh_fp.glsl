...
uniform int		u_GenericCondition;	// enable DOF debug
...
vec3 color( vec2 coords, float blur )
{
	...
	col.r = texture2D( u_ColorMap, coords + vec2(   0.0,  1.0 ) * texel * fringe * blur ).r;
	col.g = texture2D( u_ColorMap, coords + vec2(-0.866, -0.5 ) * texel * fringe * blur ).g;
	col.b = texture2D( u_ColorMap, coords + vec2( 0.866, -0.5 ) * texel * fringe * blur ).b;
...
void main( void ) 
{
	...
	vec3 col = texture2D( u_ColorMap, var_TexCoord ).rgb;
