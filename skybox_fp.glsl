...
void main( void )
{
	...
	float sun_factor = clamp( pow( dotv, pow_factor * 3.0 ), 0.0, 1.0 );
	...
	if( bool( gl_Fog.density != 0.0 ))
	{
		float fogFactor = clamp( exp2(( -gl_Fog.density * 0.25 ) * ( gl_FragCoord.z / gl_FragCoord.w )), 0.0, 1.0 );
